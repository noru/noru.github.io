...
- 主要产品是为数据团队建立新的ETL平台，同时也参与元数据管理、pipeline运维和数据治理等产品的开发。 另负责搭建部门应用托管平台，托管关键应用程序，和为不同数据团队量身定制的特定工具。此外，也为数据团队提供一些IaaS和SaaS方案.

- 基于Hadoop/Spark大数据平台对关键业务数据进行分析和可视化.

- 主导了团队内部组件库的建立, 规范化内部组件及应用的开发.

- 有一定AIGC/LLM在大数据平台相关应用的探索和应用.
...
- 持续改进和重构项⽬。包括 JSP 旧页⾯到新架构的⽆缝迁移(单页应⽤ + 微服务)；整合分散的前端⼯程，统⼀技术栈，减少维护和学习成本，并最终实现项⽬代码库的可扩展性。
